...
public class TopKDriverMapper extends Mapper<Text, Text, Text, Text> {
	...
	public void map(Text key, Text value, Context context)
        ...
        float earningsPerMin = ((float) Integer.parseFloat(outputArr[0])) / ((float) Integer.parseFloat(outputArr[1]));
	...
	public void cleanup(Context context) throws IOException, InterruptedException {
		...
		while (pq.size() > 0) {
            ...
            context.write(new Text(mostEfficientDrivers.getDriverId()), new Text(mostEfficientDrivers.getEarningsPerMin().toString()));
